...
// ── Shared write-time helper ───────────────────────────────────
// Omit read-only Timestamp fields, re-declare them as writable FieldValue
type WriteUpdate<T, K extends keyof T> = Omit<Partial<T>, K> & {
  [P in K]?: FieldValue
}
...
// ── Write-update types (Timestamp fields replaced with FieldValue) ──
type TaskUpdate     = WriteUpdate<Task,     'createdAt' | 'updatedAt' | 'completedAt'>
type ClientUpdate   = WriteUpdate<Client,   'createdAt' | 'updatedAt'>
type DealUpdate     = WriteUpdate<Deal,     'createdAt' | 'updatedAt' | 'closedAt'>
type ProposalUpdate = WriteUpdate<Proposal, 'createdAt' | 'updatedAt'>

// ── Helper: convert Firestore doc to plain object ──────────────
function docToObj(snap: DocumentData): DocumentData & { id: string } {
...
export async function createTask(data: Omit<Task, 'id'>): Promise<Task> {
  ...
  await logActivity('task_created', `Task created: ${data.title}`, {
    entityId: ref.id, entityType: 'tasks', clientName: data.clientName,
  })
...
export async function getTasks(filters: {
  ...
  // Client-side date filters (avoids composite indexes on Firestore free tier)
...
export async function updateTask(id: string, data: Partial<Task>): Promise<void> {
  const updates: TaskUpdate = { ...data, updatedAt: serverTimestamp() }
...
export async function createClient(data: Omit<Client, 'id'>): Promise<Client> {
  ...
  await logActivity('client_added', `Client added: ${data.name}`, {
    entityId: ref.id, entityType: 'clients',
  })
  return { id: ref.id, ...data }
}
...
export async function updateClient(id: string, data: Partial<Client>): Promise<void> {
  const updates: ClientUpdate = { ...data, updatedAt: serverTimestamp() }
...
export async function createDeal(data: Omit<Deal, 'id'>): Promise<Deal> {
  ...
  await logActivity('deal_created', `Deal created: ${data.title} — $${data.value?.toLocaleString()}`, {
    entityId: ref.id, entityType: 'deals', clientName: data.clientName,
  })
  return { id: ref.id, ...data }
}
...
export async function updateDeal(id: string, data: Partial<Deal>): Promise<void> {
  const updates: DealUpdate = { ...data, updatedAt: serverTimestamp() }
...
export async function updateProposal(id: string, data: Partial<Proposal>): Promise<void> {
  const updates: ProposalUpdate = { ...data, updatedAt: serverTimestamp() }
...
export async function logActivity(
  ...
  try {
    await addDoc(activityCol(), { type, description, ...meta, createdAt: serverTimestamp() })
  } catch {
    // swallow — log errors must never crash the caller
  }
}
